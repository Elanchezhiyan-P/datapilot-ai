/*
    02 - Fill the DataPilotLab tables with sample data.

    Run with an ADMIN login, after 01_create_tables.sql. Safe to re-run on its own:
    it empties the tables first and restarts their ID counters at 1.

    The data is generated from hashes, not random numbers, so every run produces
    exactly the same rows. Evaluation results and the numbers quoted in the README
    (e.g. 409 students in Coimbatore) depend on that.

    Result: 40 schools, 3,000 students, 18 exams, 5,443 registrations,
            5,443 payments, 4,886 results.
*/
USE DataPilotLab;
GO
...
-- Empty the tables, children first (foreign keys), and restart IDs at 1.
...
-- RESEED 0 makes the next ID 1, but only for a table that has held rows before.
-- A brand-new table would start at 0 instead, so it is skipped (it starts at 1 anyway).
DECLARE @table NVARCHAR(128);
DECLARE tables CURSOR LOCAL FAST_FORWARD FOR
    SELECT N'dbo.' + t.name
    FROM sys.identity_columns ic JOIN sys.tables t ON t.object_id = ic.object_id
    WHERE t.name IN (N'Schools', N'Students', N'Exams', N'Registrations', N'Payments', N'Results')
